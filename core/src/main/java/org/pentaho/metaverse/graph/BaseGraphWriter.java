...
public abstract class BaseGraphWriter implements IGraphWriter {
  ...
  /**
   * In some cases, we wre unable to make a connection between a step's output fields and the input fields that the step
   * hops to at analysis time. Inspect all input stream transformation field of a step that do not have a corresponding
   * IN "output" link, and check whether there are any matching "output" fields in the steps that hop to the given
   * step. If so, merge the fields, OR is there are multiple fields that match, add appropriate missing links.
   */
